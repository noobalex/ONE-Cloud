...
public class ValidateCodeService {
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        OneValidateCodeProperties code = properties.getCode();
        setHeader(response, code.getType());

        Captcha captcha = createCaptcha(code);
        redisService.set(OneConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()), code.getTime());
    ...
    public void check(String key, String value) throws ValidateCodeException {
        Object codeInRedis = redisService.get(OneConstant.CODE_PREFIX + key);
    ...
    private void setHeader(HttpServletResponse response, String type) {
        if (StringUtils.equalsIgnoreCase(type, OneConstant.GIF)) {
